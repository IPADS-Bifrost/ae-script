...
test() {
        ...
        for (( vm_config = $VM_CONFIG_START; vm_config < $VM_CONFIG_END; vm_config ++ )); do
                ...
                for (( data_idx = 0; data_idx < $memtier_sz_num; data_idx ++ )); do
                        ...
                        output=data/amd-memtier-${mode}-${swiotlb_opt}-${cpu_nr}vcpu-${hit_rate}-${data_sz}.stat
                ...
                for (( data_idx = 0; data_idx < $redis_sz_num; data_idx ++ )); do
                        ...
                        output=data/amd-redis-${mode}-${swiotlb_opt}-${cpu_nr}vcpu-${hit_rate}-${data_sz}.stat
                        echo "[$mode][$(date|awk '{print $5}')] set data size to ${data_sz}B" >> $RUN_LOG
                ...
                for (( data_idx = 0; data_idx < $nginx_sz_num; data_idx ++ )); do
                        ...
                        output=data/amd-nginx-${mode}-${swiotlb_opt}-${cpu_nr}vcpu-${data_sz}.stat
                        echo "[$mode][$(date|awk '{print $5}')] set data size to ${data_sz}B" >> $RUN_LOG
                        # warm up
                ...
                for (( i = 0; i < $TLS_TEST_TIMES; i ++ )); do
                        ...
                        pkill tls_server
                        stat=data/amd-tls-tx-${mode}-${swiotlb_opt}.stat
                ...
                for (( i = 0; i < $TLS_TEST_TIMES; i ++ )); do
                        ...
                        stat=data/amd-tls-rx-${mode}-${swiotlb_opt}.stat

                        ssh $SSH_VM "rm -rf /tmp/tls-*.log"
