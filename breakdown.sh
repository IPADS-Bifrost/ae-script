...
run_memtier_breakdown() {
	...
	stat=$1
        ...
        # sleep 17 && ./perf-cache.sh 2>&1 | tee $raw.perf &
        ...
        echo vmexit >> $stat-host.stat
        sed -n '/vmexit/p' $raw.host | awk '{print $4, $6, $8}' >> $stat-host.stat
        echo end >> $stat-host.stat
        rm $raw.host
}
...
test() {
        ...
        [[ $mode == "npg" || $mode == "vpg" || $mode == "vgbd" || $mode == "ngbd" || $mode == "ngnp" ]] && gro=1
        ...
        for (( vm_config = 1; vm_config < 2; vm_config ++ )); do
                ...
                data_sz=262144
                output=data/intel-memtier-${mode}-${swiotlb_opt}-breakdown
